...
DBR.prototype.findStagedDBR = function (month) {
  ...
  return Promise.all([self.getDBRs(), self.getStagedDBRs()]).then(function (values) {
    ...
    let match = _.find(stagedDBRs, function(d) {
      ...
      if (typeof d.Metadata !== 'undefined' && typeof d.Metadata.LastModified !== 'undefined' &&
        JSON.parse(JSON.stringify(originalDBR.LastModified)) === JSON.parse(d.Metadata.LastModified)) {
        // TODO: Worried here that the staged file has a LastModified date equal
        // to when we uploaded it, so it's always going to be newer than the
        // original. Not sure how to rectify this.
        console.log('found match');
        return true;
      }
      console.log('found newer file for', d.Month);
      console.log('original modified', originalDBR.LastModified);
      console.log('last modified', d.LastModified);
      console.log('d.metadata', d.Metadata);
      console.log('original.metadata', originalDBR.Metadata);
      return false;
    });
    if (match === undefined) {
...
// four properties: Key, LastModified, Size, and Month:
//   Key:  the filename
//   LastModified: the last modified date (set by S3, we can't control this)
...
function processDBRBucketContents (results) {
  ...
  for (let result of results) {
    ...
    let picked = _.pick(result, ['Key', 'Size', 'LastModified', 'Metadata']);
...
function processDBR (dbr, s3client, bucket) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var requestParams = {
      ...
      Metadata: {
        LastModified: JSON.stringify(dbr.LastModified),
      },
    }
