  ...
  let query = `-- segmentio/awsbilling/lib/redshift.js:importFullMonth
    BEGIN;
      ...
      COPY ${this.schema}.${stagingTableName}
        FROM '${s3uri}'
        CREDENTIALS '${this.credentialString()}'
      ...
      INSERT INTO ${this.schema}.${this.lineItemsTableName} (
        ${selectStatement.join(',\n')}
      ) SELECT ${selectStatement.join(',\n')}
       FROM ${this.schema}.${stagingTableName};
  ...
  return this.executeQuery(truncateQuery).then(function () {
    ...
    let query = `
      COPY ${self.schema}.month_to_date
        FROM '${s3uri}'
        CREDENTIALS '${self.credentialString()}'
        GZIP CSV IGNOREHEADER 1;
    `
...
Redshift.prototype.credentialString = function() {
  ...
  var credentialParts = [];
  if (typeof this.s3.config.credentials.accessKeyId === 'string') {
    credentialParts.push('aws_access_key_id=' + this.s3.config.credentials.accessKeyId);
  }
  if (typeof this.s3.config.credentials.secretAccessKey === 'string') {
    credentialParts.push('aws_secret_access_key=' + this.s3.config.credentials.secretAccessKey);
  }
  if (typeof this.s3.config.credentials.sessionToken === 'string') {
    credentialParts.push('token=' + this.s3.config.credentials.sessionToken);
  }
